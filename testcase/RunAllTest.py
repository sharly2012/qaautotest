...
def create_suite():
    ...
    test_dir = GlobalVar.get_root_path() + '/testcase/'
...
def report():
    if len(sys.argv) > 1:
        report_name = GlobalVar.get_root_path() + '/report/' + sys.argv[1] + '_result.html'
    else:
        now = time.strftime("%Y-%m-%d_%H_%M_%S_")
        report_name = GlobalVar.get_root_path() + '/report/' + now + 'result.html'
